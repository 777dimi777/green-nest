...
const items = [
  { icon: LayoutDashboard, label: "Pregled" },
  { icon: Boxes, label: "Proizvodi" },
...
export default function AdminLayout({
  ...
  return (
    <AdminRoute>
      <div className="min-h-screen bg-muted/35 md:grid md:grid-cols-[240px_1fr]">
        <aside className="border-b bg-card p-5 md:min-h-screen md:border-b-0 md:border-r">
          ...
          <nav
            ...
            className="mt-8 flex gap-2 overflow-x-auto md:flex-col"
          >
            {items.map(({ icon: Icon, label }, index) => (
              <span
                key={label}
                className={`flex min-w-max items-center gap-3 rounded-lg px-3 py-2 text-sm ${
                  index === 0
