...
export function ProductEditor({ id }: { id?: string }) {
  const product = useQuery({
    queryKey: adminQueryKeys.products.detail(id ?? "new"),
    queryFn: () => adminApi.products.detail(id as string),
    enabled: !!id,
  });
  ...
  if (id && product.isError)
    return (
      <ErrorState
        title="Proizvod nije dostupan"
        description={getApiErrorMessage(product.error)}
      />
    );
...
function ProductForm({ product }: { product?: ProductDetails }) {
  ...
  const save = useMutation({
    mutationFn: (payload: object) =>
      product
        ? adminApi.products.update({ id: product.id, payload })
        : adminApi.products.create(payload),
    onSuccess: async (created) => {
      toast.success("Proizvod je sačuvan.");
      await client.invalidateQueries({ queryKey: adminQueryKeys.products.all });
      for (const [index, file] of files.entries())
        await adminApi.products.upload({
          id: created.id,
          file,
          isPrimary: index === 0 && !created.images.length,
        });
  ...
  const imageAction = useMutation({
    mutationFn: async (action: {
      ...
      action.type === "delete"
        ? adminApi.products.removeImage({
            productId: product!.id,
            imageId: action.imageId,
          })
        : adminApi.products.setPrimary({
            productId: product!.id,
            imageId: action.imageId,
          }),
    onSuccess: async () => {
      toast.success("Slike su ažurirane.");
      await client.invalidateQueries({
        queryKey: adminQueryKeys.products.detail(product?.id ?? ""),
      });
    },
    onError: (e) => toast.error(getApiErrorMessage(e)),
  });
  ...
  return (
    <>
      <p className="text-sm text-primary">Admin / Proizvodi</p>
      <h1 className="mt-2 font-serif text-4xl font-semibold">
        {product ? "Izmena proizvoda" : "Novi proizvod"}
      </h1>
      <form
        className="mt-8 grid max-w-4xl gap-4 sm:grid-cols-2"
      ...
      >
        ...
        <div>
          ...
          {errors.categoryId && (
            <p className="text-xs text-destructive">Izaberite kategoriju.</p>
          )}
        </div>
        ...
        <div className="sm:col-span-2">
          <Label htmlFor="product-images">Nove slike</Label>
          <Input
            id="product-images"
            type="file"
            multiple
        ...
        <Button className="sm:col-span-2" disabled={save.isPending}>
          {save.isPending ? "Čuvanje…" : "Sačuvaj proizvod"}
        </Button>
      </form>
      {product && product.images.length > 0 && (
        <section className="mt-10 max-w-4xl">
          <h2 className="font-serif text-2xl font-semibold">Postojeće slike</h2>
