...
export function WishlistPage() {
  ...
  if (query.isError)
    return (
      <ErrorState
        title="Lista želja nije dostupna"
  ...
  return (
    <>
      <div className="mb-8">
        <h1 className="font-serif text-5xl font-semibold">Lista želja</h1>
        <p className="mt-2 text-muted-foreground">
          {query.data.total} sačuvanih proizvoda
        </p>
      </div>
      {query.data.data.length === 0 ? (
        <EmptyState
          ...
          description="Sačuvajte biljke koje želite da pogledate kasnije."
          action={
