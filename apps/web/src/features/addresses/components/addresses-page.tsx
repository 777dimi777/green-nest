...
import { useAddresses, useRemoveAddress } from "../hooks/use-addresses";
import { AddressForm } from "./address-form";
export function AddressesPage() {
  const q = useAddresses(),
    remove = useRemoveAddress(),
  ...
  return (
    <>
      <div className="mb-8 flex items-end justify-between">
        <div>
          <h1 className="font-serif text-5xl font-semibold">Adrese</h1>
          <p className="mt-2 text-muted-foreground">
            Adrese za dostavu porudžbina.
          </p>
        </div>
        <Button onClick={() => setAdding((v) => !v)}>
          {adding ? "Zatvori" : "Nova adresa"}
        </Button>
      </div>
      {adding && (
          ...
          <article key={a.id} className="rounded-xl border p-5">
            ...
            <div className="mt-4 flex gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  if (window.confirm("Obrisati adresu?")) remove.mutate(a.id);
                }}
              >
                <Trash2 />
                Obriši
